//! Event-driven public Hyperliquid ingestion.
//!
...
//! reconciliation source; each wallet independently regains eligibility
//! after its post-gap baseline, without a cohort-wide completion barrier.
...
const MAX_WS_SUBSCRIPTIONS: usize = 1_000;
// Public subscriptions use at most 979 slots. Keep the remaining slots out
// of this actor's budget for account subscriptions/connection transitions.
const RESERVED_WS_SUBSCRIPTIONS: usize = 21;
...
pub enum StreamingEvent {
    ...
    MarketFlow {
        asset: String,
        flow: crate::mfce_delayed::MarketFlow,
    },
    AssetContexts(Value),
...
fn route_public_trades(
    trades: &mut Vec<PublicTrade>,
    markets: &BTreeSet<String>,
    tracked_wallets: &BTreeSet<String>,
    flow: &mut BTreeMap<String, crate::mfce_delayed::MarketFlow>,
    events: &mpsc::Sender<StreamingEvent>,
) {
    // Flow uses every public trade; wallet attribution is a separate filter.
    for trade in trades.iter().filter(|trade| markets.contains(&trade.coin)) {
        let state = flow.entry(trade.coin.clone()).or_default();
        if let Some(notional) = trade.price.checked_mul(trade.size) {
            state.trade(trade.time_ms, trade.trade_id, notional, trade.aggressor_buy);
        }
        if trade.time_ms.saturating_sub(state.last_sent_ms) >= 1_000 {
            state.last_sent_ms = trade.time_ms;
            // Cumulative bounded snapshots may be shed; source ingestion never
            // waits for a learning consumer.
            let _ = events.try_send(StreamingEvent::MarketFlow {
    ...
    trades.retain(|trade| {
        tracked_wallets.contains(&trade.buyer) || tracked_wallets.contains(&trade.seller)
    });
}
...
async fn run_stream(
    ...
    loop {
        ...
        let mut flow = BTreeMap::<String, crate::mfce_delayed::MarketFlow>::new();
        ...
        let disconnected = loop {
            tokio::select! {
                command = commands.recv() => {
                    match command {
                        Some(StreamingCommand::ReplaceMarkets(next)) => {
                            ...
                            flow.retain(|asset, _| next.contains(asset));
                            markets = next;
                        }
                            ...
                            Ok(Some(mut event)) => {
                                match &mut event {
                                    StreamingEvent::Trades(trades) => {
                                        ...
                                        route_public_trades(trades, &markets, &tracked_wallets, &mut flow, &events);
                                        if trades.is_empty() {
                                            continue;
                                        }
                                        metrics
                ...
                _ = watchdog.tick() => {
                    if check_transport(&mut writer, last_received.elapsed(), policy.idle_timeout_ms).await.is_err() {
                        break true;
                    }
                }
            }
...
{
    if silence > Duration::from_millis(idle_timeout_ms) {
        return Err(StreamingError::ChannelClosed);
    }
    // Only receives renew continuity. Sending a ping never hides a dead socket.
    if silence >= Duration::from_millis(idle_timeout_ms / 3) {
        writer
    ...
    rows.iter()
        .map(|row| {
            ...
            Ok(PublicTrade {
                ...
                aggressor_buy: match string_field(row, "side")? {
                    "B" => true,
                    "A" => false,
                    _ => return Err(StreamingError::InvalidPayload),
                },
...
pub struct SourceStateUpdate {
    pub state: SourceStateResponse,
}
...
impl StreamingSourceBook {
    ...
    pub fn install_baseline(
        ...
        self.recovering_wallets.remove(&baseline.candidate_id);
        Ok(baseline)
    }

    /// Hydrates restart recovery without granting current-process readiness.
    /// This is called before stream processing starts, so no buffered event is
    /// allowed to be folded into a merely durable baseline.
    pub fn restore_durable_baseline(
        &mut self,
        mut baseline: SourceStateResponse,
    ) -> Result<(), StreamingError> {
        let wallet = baseline.candidate_id.to_ascii_lowercase();
        if !self.tracked.contains(&wallet) || self.states.contains_key(&wallet) {
            return Err(StreamingError::InvalidPayload);
        }
        baseline.candidate_id = wallet.clone();
        baseline.closed_candles.clear();
        self.states.insert(wallet, baseline);
        Ok(())
    }
    ...
    pub fn apply_trade(
        &mut self,
        trade: PublicTrade,
    ) -> Result<Vec<SourceStateUpdate>, StreamingError> {
            ...
            if !self.recovering_wallets.contains(&fill.wallet) {
                if let Some(state) = self.states.get_mut(&fill.wallet) {
                    if fill.time_ms >= state.source_time_ms {
                        ...
                        changed.push(SourceStateUpdate {
                            state: state.clone(),
                        });
                    }
                }
    ...
    pub fn begin_recovery(&mut self) {
        self.recovering_wallets = self.states.keys().cloned().collect();
...
fn validate_markets(markets: &BTreeSet<String>) -> Result<(), StreamingError> {
    if markets.is_empty()
        ...
        || markets
            ...
            .saturating_add(1)
            .saturating_add(RESERVED_WS_SUBSCRIPTIONS)
...
mod tests {
    ...
    fn buffered_trade_after_baseline_is_applied_once() {
        ...
        assert_eq!(book.hydrated_count(), 2);
        assert!(book.apply_trade(trade(101, 1)).unwrap().is_empty());
    }
    ...
    fn tracked_trade_updates_open_reduce_and_cross() {
        ...
        assert_eq!(
            first.state.positions["xyz:XYZ100"].signed_size,
            Decimal::from(2)
        );
        ...
        assert_eq!(
            crossed.state.positions["xyz:XYZ100"].signed_size,
            Decimal::from(-1)
        );
        assert_eq!(
            crossed.state.positions["xyz:XYZ100"].entry_price,
            Some(Decimal::from(10))
        );
    }

    #[test]
    fn durable_restore_hydrates_without_claiming_missing_wallet_recovery() {
        let restored = wallet('a');
        let missing = wallet('b');
        let mut book = StreamingSourceBook::new([restored.clone(), missing]).unwrap();
        book.restore_durable_baseline(baseline(restored.clone(), 100))
            .unwrap();
        assert_eq!(book.hydrated_count(), 1);

        book.begin_recovery();
        let ending = baseline(restored.clone(), 101);
        book.install_baseline(ending).unwrap();
        assert!(book.recovering_wallets.is_empty());
    }
    ...
    fn streamed_fill_retains_raw_position_from_zero_equity_empty_baseline() {
        ...
        assert_eq!(changed.state.account_value, Decimal::ZERO);
        assert_eq!(
            changed.state.positions["xyz:XYZ100"].signed_notional,
            Decimal::from(20)
        );
    }

    #[test]
    fn authoritative_reconciliation_folds_only_newer_buffered_stream_events() {
        let candidate = wallet('a');
        ...
        assert!(book
            .apply_trade(live_after_cutoff.clone())
            .unwrap()
            .is_empty());

        let current = book
            .install_baseline(baseline(candidate.clone(), 102))
            .unwrap();
        assert_eq!(
            current.positions["xyz:XYZ100"].signed_size,
            Decimal::from(2)
        );
        assert_eq!(
            book.recovering_wallets,
            BTreeSet::from([unresolved.clone()])
        );
        ...
        book.install_baseline(baseline(unresolved, 102)).unwrap();
        assert!(book.recovering_wallets.is_empty());
        assert!(book.apply_trade(live_after_cutoff).unwrap().is_empty());
    }

    #[test]
    fn trade_direction_uses_buyer_seller_not_aggressor_side() {
        for side in ["B", "A"] {
            let message = json!({
                "channel":"trades",
                "data":[{
                    "coin":"xyz:XYZ100","side":side,"px":"12.5","sz":"3",
            ...
            for tracked in [
                ...
                let mut book = StreamingSourceBook::new(tracked.clone()).unwrap();
                for address in &tracked {
                    book.install_baseline(baseline(address.clone(), 122))
                        .unwrap();
                }
                let updates = book.apply_trade(trades[0].clone()).unwrap();
                assert_eq!(updates.len(), tracked.len());
                for update in updates {
                    let expected = if update.state.candidate_id == wallet('a') {
                        3
                    } else {
                        -3
                    };
                    assert_eq!(
                        update.state.positions["xyz:XYZ100"].signed_size,
                        Decimal::from(expected)
                    );
                }
                assert!(book.apply_trade(trades[0].clone()).unwrap().is_empty());
            }
        }
    }

    #[test]
    fn untracked_trade_updates_flow_but_never_source_provenance() {
        let (sender, mut receiver) = mpsc::channel(4);
        let mut trades = vec![trade(60_000, 1)];
        let mut flow = BTreeMap::new();
        route_public_trades(
            &mut trades,
            &BTreeSet::from(["xyz:XYZ100".into()]),
            &BTreeSet::from([wallet('c')]),
            &mut flow,
            &sender,
        );
        assert!(trades.is_empty(), "no tracked wallet, no SourceState event");
        let StreamingEvent::MarketFlow { asset, flow } = receiver.try_recv().unwrap() else {
            panic!("untracked trade must still produce flow");
        };
        assert_eq!(asset, "xyz:XYZ100");
        assert!(flow.features(60_000).unwrap()[0] > Decimal::ZERO);
        assert!(receiver.try_recv().is_err());
    }

    #[tokio::test]
    async fn heartbeat_keeps_quiet_markets_healthy_but_sending_does_not_reset_silence() {
        let messages = std::sync::Mutex::new(Vec::new());
        let mut writer = futures_util::sink::unfold(&messages, |messages, message| {
            messages.lock().unwrap().push(message);
            std::future::ready(Ok::<_, StreamingError>(messages))
        });
        // No trades for a minute; inbound pong every five seconds renews the
        // same receive clock as trades, contexts and subscription messages.
        for _ in 0..12 {
            check_transport(&mut writer, Duration::from_secs(5), 15_000)
                .await
                .unwrap();
            assert!(parse_message(r#"{"channel":"pong"}"#).unwrap().is_none());
            check_transport(&mut writer, Duration::ZERO, 15_000)
                .await
                .unwrap();
        }
        ...
        assert!(messages
            ...
            .iter()
            .all(
                |message: &Message| serde_json::from_str::<Value>(message.to_text().unwrap())
                    .unwrap()["method"]
                    == "ping"
            ));
        assert!(
            check_transport(&mut writer, Duration::from_secs(16), 15_000)
                .await
                .is_err()
        );
    }
    ...
    #[tokio::test]
    async fn subscription_rotation_unsubscribes_before_using_reserved_slots() {
        let current = (0..MAX_MARKETS)
            .map(|index| format!("M{index}"))
            .collect::<BTreeSet<_>>();
        let next = (MAX_MARKETS..MAX_MARKETS * 2)
            .map(|index| format!("M{index}"))
            .collect::<BTreeSet<_>>();
        validate_markets(&current).unwrap();
        let mut oversized = current.clone();
        oversized.insert("extra".into());
        assert_eq!(validate_markets(&oversized), Err(StreamingError::Capacity));
        let slots =
            std::sync::Mutex::new(MAX_MARKETS + MAX_HOT_BOOKS + 1 + RESERVED_WS_SUBSCRIPTIONS);
        let mut writer = futures_util::sink::unfold(&slots, |slots, message: Message| {
            let value: Value = serde_json::from_str(message.to_text().unwrap()).unwrap();
            let mut count = slots.lock().unwrap();
            if value["method"] == "unsubscribe" {
                *count -= 1;
            } else {
                *count += 1;
            }
            assert!(*count <= MAX_WS_SUBSCRIPTIONS);
            std::future::ready(Ok::<_, StreamingError>(slots))
        });
        replace_subscriptions(&mut writer, "trades", &current, &next)
            .await
            .unwrap();
        assert_eq!(*slots.lock().unwrap(), MAX_WS_SUBSCRIPTIONS);
    }
