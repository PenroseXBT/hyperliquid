...
//! Bounded, observer-only LightGBM quantile models for MFCE1.
//!
...
//! Rust-owned data back to the observer runtime.
...
pub const ENGINE_VERSION: &str = "MFCE1-LIGHTGBM-4.6.0-QPAIR-V1";
...
pub const MIN_TRAINING_ROWS: usize = 64;
...
const MIN_DATA_IN_LEAF: u32 = 20;
const MODEL_SEED: u32 = 1_729;
const PREDICTION_PARAMETERS: &str = "num_threads=1";

/// One of the two conditional gross-return quantiles modeled by MFCE1.
...
/// One row of conditional gross-transition-return predictions.
...
pub fn train_quantile_pair(
    features: &[f64],
    gross_transition_returns: &[f64],
    weights: Option<&[f64]>,
    feature_count: usize,
) -> Result<QuantileModelStrings, MfceError> {
    let rows = validate_training_data(features, gross_transition_returns, weights, feature_count)?;
    let labels = to_f32_labels(gross_transition_returns)?;
