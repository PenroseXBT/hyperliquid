...
use crate::consensus::ConsensusInput;
use crate::decision::{
...
use crate::execution_floor::ExecutionFloorPolicy;
use crate::portfolio_risk::{MarketRules, PortfolioProjectionInput};
use crate::shadow::{
    execute_shadow_ioc, DepthLevel, LatencyScenario, ShadowExecution, ShadowExecutionInput,
...
pub fn execute_fixture_shadow(
    plan: &FixturePlan,
    taker_fee_bps: f64,
) -> Result<Vec<ShadowExecution>, FixtureError> {
    ...
    plan.actions
        ...
        .map(|action| {
            ...
            let decision_snapshot = ShadowMarketSnapshot {
            ...
            let evaluation_snapshot = ShadowMarketSnapshot {
                snapshot_id: hash_market_snapshot_bytes(
            ...
            execute_shadow_ioc(&ShadowExecutionInput {
                action: action.clone(),
                ...
                proposed_limit_price: match action.side {
                    crate::decision::Side::Buy => midpoint + limit_buffer,
                    crate::decision::Side::Sell => midpoint - limit_buffer,
                },
