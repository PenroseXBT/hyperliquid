use crate::decision::{MarketSnapshotId, PlannedAction, Side};
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ShadowMarketSnapshot {
...
#[derive(Debug, Clone)]
pub struct ShadowExecutionInput {
    pub action: PlannedAction,
    pub decision_timestamp_mono: u64,
    pub decision_market_snapshot: ShadowMarketSnapshot,
    pub evaluation_market_snapshot: ShadowMarketSnapshot,
...
pub struct ShadowExecutionId(pub [u8; 32]);

impl Display for ShadowExecutionId {
...
pub struct ShadowExecution {
    pub shadow_execution_id: ShadowExecutionId,
    pub action: PlannedAction,
    pub decision_timestamp_mono: u64,
...
/// Builds the capped, marketable IOC price used by both shadow qualification
/// and a future live executor. Complete visible depth is preferred. If the
...
pub fn plan_marketable_ioc(
    side: Side,
    quantity: Decimal,
    evaluation_market_snapshot: &ShadowMarketSnapshot,
    reference_price: Decimal,
    execution_cushion: Decimal,
    maximum_slippage: Decimal,
    price_tick: Decimal,
) -> Result<MarketableIocPlan, ShadowError> {
    ...
    {
        return Err(ShadowError::InvalidInput("marketable IOC pricing inputs"));
    }
    ...
    for level in levels {
        visible = visible
            ...
            .ok_or(ShadowError::ArithmeticOverflow("visible depth"))?;
        if visible >= quantity {
    ...
    .ok_or(ShadowError::ArithmeticOverflow("maximum IOC price"))?;
    ...
    let (limit_price, pricing_mode) = if let Some(worst_price) = worst_required {
        ...
        .ok_or(ShadowError::ArithmeticOverflow("depth IOC price"))?;
        (
    ...
    let visible_executable_quantity = levels
        ...
        .ok_or(ShadowError::ArithmeticOverflow("executable visible depth"))?
        .min(quantity);
...
fn round_price_outward(value: Decimal, tick: Decimal, side: Side) -> Result<Decimal, ShadowError> {
    if value <= Decimal::ZERO || tick <= Decimal::ZERO {
        return Err(ShadowError::InvalidInput("price and tick"));
    }
    let units = value
        .checked_div(tick)
        .ok_or(ShadowError::ArithmeticOverflow("price units"))?;
    ...
    units
        .checked_mul(tick)
        .ok_or(ShadowError::ArithmeticOverflow("rounded IOC price"))
}

pub fn execute_shadow_ioc(input: &ShadowExecutionInput) -> Result<ShadowExecution, ShadowError> {
    validate_input(input)?;
    let expected_evaluation_at = input
        .decision_timestamp_mono
        .checked_add(input.configured_latency_ms)
        .ok_or(ShadowError::ArithmeticOverflow("latency timestamp"))?;
    if input.evaluation_market_snapshot.observed_at_mono < expected_evaluation_at {
        return Err(ShadowError::SnapshotTooEarly);
    }

    let executable = match input.action.side {
        Side::Buy => &input.evaluation_market_snapshot.asks,
        Side::Sell => &input.evaluation_market_snapshot.bids,
    };
    let mut remaining = input.rounded_quantity;
    let mut filled = Decimal::ZERO;
    let mut fill_value = Decimal::ZERO;
    for level in executable {
        let within_limit = match input.action.side {
            Side::Buy => level.price <= input.proposed_limit_price,
            Side::Sell => level.price >= input.proposed_limit_price,
        };
        if !within_limit || remaining.is_zero() {
            break;
        }
        let quantity = remaining.min(level.quantity);
        filled = filled
            .checked_add(quantity)
            .ok_or(ShadowError::ArithmeticOverflow("filled quantity"))?;
        fill_value = fill_value
            .checked_add(
                quantity
                    .checked_mul(level.price)
                    .ok_or(ShadowError::ArithmeticOverflow("fill value"))?,
            )
            .ok_or(ShadowError::ArithmeticOverflow("fill value sum"))?;
        remaining = remaining
            .checked_sub(quantity)
            .ok_or(ShadowError::ArithmeticOverflow("IOC remainder"))?;
    }
    let average = if filled.is_zero() {
        ...
        Some(
            fill_value
                .checked_div(filled)
                .ok_or(ShadowError::ArithmeticOverflow("average fill price"))?,
        )
    };
    let fees = fill_value
        .checked_mul(input.taker_fee_rate)
        .ok_or(ShadowError::ArithmeticOverflow("fees"))?;
    let slippage = match average {
        ...
        Some(average) => {
            ...
            .ok_or(ShadowError::ArithmeticOverflow("slippage price"))?;
            price_cost
                .checked_mul(filled)
                .ok_or(ShadowError::ArithmeticOverflow("slippage"))?
        }
    };
    ...
    let position_after = input
        ...
        .ok_or(ShadowError::ArithmeticOverflow("position after"))?;
    Ok(ShadowExecution {
        shadow_execution_id: derive_shadow_execution_id(input),
...
pub fn execute_latency_scenarios(
    scenarios: BTreeMap<LatencyScenario, ShadowExecutionInput>,
) -> Result<BTreeMap<LatencyScenario, ShadowExecution>, ShadowError> {
    scenarios
        .into_iter()
        .map(|(scenario, mut input)| {
            input.latency_scenario = scenario;
            Ok((scenario, execute_shadow_ioc(&input)?))
        })
        .collect()
}

fn validate_input(input: &ShadowExecutionInput) -> Result<(), ShadowError> {
...
fn validate_book(snapshot: &ShadowMarketSnapshot) -> Result<(), ShadowError> {
    for levels in [&snapshot.bids, &snapshot.asks] {
        if levels
            .iter()
            .any(|level| level.price <= Decimal::ZERO || level.quantity <= Decimal::ZERO)
        {
            return Err(ShadowError::InvalidBook("non-positive depth"));
        }
    }
    if snapshot
        .bids
    ...
    {
        return Err(ShadowError::InvalidBook("bids must be strictly descending"));
    }
    if snapshot
        .asks
        .windows(2)
        .any(|pair| pair[0].price >= pair[1].price)
    {
        return Err(ShadowError::InvalidBook("asks must be strictly ascending"));
    }
    Ok(())
}

fn derive_shadow_execution_id(input: &ShadowExecutionInput) -> ShadowExecutionId {
    ...
    ShadowExecutionId(hash.finalize().into())
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::decision::{DecisionId, PlannedCloid, TargetVersion};
    use std::str::FromStr;
    ...
    fn snapshot(id: u8, observed: u64) -> ShadowMarketSnapshot {
    ...
    fn input(side: Side) -> ShadowExecutionInput {
    ...
    fn deterministic_ioc_walks_depth_and_cancels_remainder() {
        ...
        let execution = execute_shadow_ioc(&input).unwrap();
        ...
        assert_eq!(execution, execute_shadow_ioc(&input).unwrap());
    }

    #[test]
    fn sell_side_fees_funding_and_slippage_are_explicit() {
        let execution = execute_shadow_ioc(&input(Side::Sell)).unwrap();
    ...
    fn fixed_scenarios_are_all_retained_without_best_case_selection() {
        ...
        assert_ne!(
            results[&LatencyScenario::Optimistic].shadow_execution_id,
            results[&LatencyScenario::Conservative].shadow_execution_id
        );
    }
